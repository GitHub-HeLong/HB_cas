...
import org.jasig.cas.enumeration.LogoutWay;
import org.jasig.cas.log.LogManager;
...
public final class CentralAuthenticationServiceImpl implements CentralAuthenticationService {
    ...
    private static final int Credential = 0;
    ...
    @Override
    public List<LogoutRequest> destroyTicketGrantingTicket(final String ticketGrantingTicketId,LogoutWay logoutWay) {
        Assert.notNull(ticketGrantingTicketId);
        logger.info("Remove ticket [{}] from registry start.", ticketGrantingTicketId);
        try {
            ...
            if(ticketGrantingTicket != null){
            	...
            	logout(ticketId,userName,logoutWay);
            	//authenticationManager.updateUserOnlineState(0, userName);
            	logger.info("Removing userName {} from registry.", userName);
            }
		} catch (Exception e) {
			e.printStackTrace();
		}
        logger.info("Removing ticket [{}] from registry.", ticketGrantingTicketId);
        final TicketGrantingTicket ticket = this.ticketRegistry.getTicket(ticketGrantingTicketId,
                TicketGrantingTicket.class);

        if (ticket == null) {
            logger.info("TicketGrantingTicket [{}] cannot be found in the ticket registry.", ticketGrantingTicketId);
            return Collections.emptyList();
        }

        logger.info("Ticket found. Processing logout requests and then deleting the ticket...");
    ...
    public String grantServiceTicket(
        ...
        synchronized (ticketGrantingTicket) {
            if (ticketGrantingTicket.isExpired()) {
                this.ticketRegistry.deleteTicket(ticketGrantingTicketId);
                logger.info("TicketGrantingTicket[{}] has expired and is now deleted from the ticket registry.", ticketGrantingTicketId);
                throw new InvalidTicketException(ticketGrantingTicketId);
            }
        }
            ...
            for (Ticket ticket : tickets) {
            	try {
                	...
                	if(grantingTicket == null){
                    	...
                    	TicketGrantingTicket newTicketGrantingTicket = this.ticketRegistry.getTicket(ticketId, TicketGrantingTicket.class);
                    	Authentication authentication = newTicketGrantingTicket.getAuthentication();
                    	Principal principal = authentication.getPrincipal();
                    	String userName = principal.getId();
                    	logger.info("the ticketId is {},the ticketGrantingTicketId is {}.", ticketId,ticketGrantingTicketId);
                    	if(!ticketId.equals(ticketGrantingTicketId)&&userName.equals(principalId)){  //异地登陆T掉前一个登陆信息
                    		this.destroyTicketGrantingTicket(ticketId,LogoutWay.kicked);
                    		authenticationManager.updateUserOnlineState(1, userName);
                    	}
                    	else if(ticket.isExpired()){
                    		this.destroyTicketGrantingTicket(ticketId,LogoutWay.expired);
						}
    ...
    public String delegateTicketGrantingTicket(final String serviceTicketId, final Credential... credentials)
        ...
        if (serviceTicket == null || serviceTicket.isExpired()) {
            logger.info("ServiceTicket [{}] has expired or cannot be found in the ticket registry", serviceTicketId);
    ...
    public boolean login(String id,String username,String clientIp,String serverIp,String sysType){
    	LoginLog log = new LoginLog(id, username, clientIp, serverIp, sysType, getTime(), "online");
    	logger.info("user {} login,the loginlog's info is {}.",username,log.toString());
    	return logManager.addLoginLog(log);
    }
    
    public boolean logout(String id,String username,LogoutWay logoutWay){
    	LoginLog log = new LoginLog(id,username,"offline",getTime(),logoutWay.getLogoutType(),logoutWay.getLogoutCode());
    	logger.info("user {} logout,the loginlog's info is {}.",username,log.toString());
    	return logManager.updateLoginLog(log);
    }
    ...
    public Assertion validateServiceTicket(final String serviceTicketId, final Service service) throws TicketException {
        Assert.notNull(serviceTicketId, "serviceTicketId cannot be null");
        Assert.notNull(service, "service cannot be null");
        String clientIp = service.getClientIp();
        final ServiceTicket serviceTicket =  this.serviceTicketRegistry.getTicket(serviceTicketId, ServiceTicket.class);

        if (serviceTicket == null) {
            logger.debug("ServiceTicket [{}] does not exist.", serviceTicketId);
            throw new InvalidTicketException(serviceTicketId);
        }
        ...
        try {
            synchronized (serviceTicket) {
                if (serviceTicket.isExpired()) {
                    logger.debug("ServiceTicket [{}] has expired.", serviceTicketId);
                    throw new InvalidTicketException(serviceTicketId);
                }

                if (!serviceTicket.isValidFor(service)) {
                    logger.debug("ServiceTicket [{}] with service [{}] does not match supplied service [{}]",
            ...
            login(root.getId(),principal.getId(),clientIp,null,null);
