...
public final class TicketGrantingTicketExpirationPolicy implements ExpirationPolicy, InitializingBean, Serializable {
    ...
    public boolean isExpired(final TicketState ticketState) {
    	if(maxTimeToLiveInMilliSeconds != -1000){
    		// Ticket has been used, check maxTimeToLive (hard window)
    		long time = System.currentTimeMillis() - ticketState.getCreationTime();
    		LOGGER.info("The time since creation is {}",time);
    		LOGGER.info("The maxTimeToLiveInMilliSeconds is {}",maxTimeToLiveInMilliSeconds);
    		if ((time >= maxTimeToLiveInMilliSeconds)) {
                LOGGER.info("Ticket is expired because the time since creation is greater than maxTimeToLiveInMilliSeconds,the time since creation is {},the maxTimeToLiveInMilliSeconds is {}",time,maxTimeToLiveInMilliSeconds);
                return true;
            }
    	}
    	else {
        ...
        if(timeToKillInMilliSeconds != -1000){
        	// Ticket is within hard window, check timeToKill (sliding window)
        	long time = System.currentTimeMillis() - ticketState.getLastTimeUsed();
        	LOGGER.info("The time since last use is {}",time);
        	LOGGER.info("The timeToKillInMilliSeconds is {}",timeToKillInMilliSeconds);
            if ((time >= timeToKillInMilliSeconds)) {
                LOGGER.info("Ticket is expired because the time since last use is greater than timeToKillInMilliseconds,the time since last use is {},the timeToKillInMilliseconds is {}",time,timeToKillInMilliSeconds);
                return true;
            }
        }
        else {
